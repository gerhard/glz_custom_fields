...
function glz_custom_fields_search_form($atts) {
  ...
  if ( $_POST AND in_array("glz_custom_fields_search", $_POST) ) {
    ...
    foreach ($_POST as $key => $value)
      $_POST[$key] = (is_array($value)) ? stripslashes(join("|", $value)) : stripslashes($value);
  }
  else
    // need to create an empty array if this page isn't generated from glz_custom_fields_search
    $incoming = array();
