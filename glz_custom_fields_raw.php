...
@author Gerhard Lazu
@version 1.2
@copyright Gerhard Lazu, 21 January, 2009
...
function glz_custom_fields_search_form($atts) {
  ...
  if ( $_POST AND in_array("glz_custom_fields_search", $_POST) ) {
    ...
    foreach ($_POST as $key => $value)
      $incoming[$key] = (is_array($value)) ? stripslashes(join(",", $value)) : stripslashes($value);
  }
  // DEBUG
  // dmp($incoming);
  ...
  if ( !empty($searchby) ) {
    ...
    // there will be default sections & categories
    foreach (array("section", "category") as $value) {
      $values = (($$value == "all") ? glz_all_searchable_sections_categories($value, $$value) : $$value);
      $out[] = hInput("{$value}['default']", $values);
    }
